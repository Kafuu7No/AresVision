...
const zh = {
  ...
  common: {
    ...
    negative: '负相关',
    personalWarmup: {
      title: 'Personal 数据源预热',
      defaultMessage: '系统正在后台准备 personal 数据源与相关页面缓存。',
      stage: {
        idle: '未开始',
        queued: '已排队',
        buildingCache: '构建个人源',
  ...
  explore: {
    ...
    pageDesc: '数据管理页的核心任务是把用户上传的 OpenMARS 或 MCD 数据，经过校验、处理与切换，变成可供数据概览、数据分析、预测分析和模型训练使用的个人数据源。',
    stepUploadTitle: '上传数据集',
    stepUploadDesc: '上传 OpenMARS 或 MCD 的 `.nc` 文件，系统先识别类型、火星年、Ls 范围和网格结构。',
    stepProcessTitle: '系统处理与切换',
    stepProcessDesc: '系统完成校验后，会构建个人数据源；若个人 OpenMARS 不完整，则自动切换为“系统 OpenMARS + 个人 MCD”的混合模式。',
    stepUseTitle: '在其他页面使用',
    stepUseDesc: '处理完成后，个人数据会被其他页面作为 `personal` 数据源调用，用于可视化、分析、预测和训练。',
    defaultDataset: {
      header: '官方数据资产 OFFICIAL DATA ASSETS',
    ...
    upload: {
      ...
      loginBtn: '登录',
      title: '上传火星观测数据',
      subtitle: '支持 OpenMARS (.nc) 和 MCD (.nc) 格式。系统会自动校验数据类型、火星年、Ls 范围、变量和网格分辨率，并尝试把它接入个人数据源。',
    ...
    myData: {
      title: '我的上传记录 MY UPLOADS',
      count: ({ n }) => `共 ${n} 个数据集`,
      emptyHint: '还没有上传数据，拖拽 .nc 文件到上方区域开始',
      ingestTitle: '数据接入工作台 DATA INGESTION WORKBENCH',
      ingestDesc: '这里不做复杂治理，只负责一件事：把你上传的数据变成站内其他页面可以调用的个人数据源。你最需要关注的是上传是否通过、个人数据源是否构建完成、以及当前哪些数据真正参与了系统使用。',
      ingestCheck1: '识别类型',
      ingestCheck2: '校验网格',
      ingestCheck3: '构建个人数据源',
      statUploads: '上传记录',
      statUploadsDesc: '你当前上传到平台的全部数据文件。',
      statUsable: '当前可用',
      statUsableDesc: '已经真正接入个人数据源、可被其他页面使用的数据。',
      statYears: '激活火星年',
      statYearsDesc: '当前个人或混合数据源已支持的火星年数量。',
      currentSourceTitle: '当前个人数据源状态 CURRENT PERSONAL SOURCE',
      currentSourceBuild: '构建状态',
      currentSourceBuildDesc: '上传后系统会在后台整理并构建个人数据源缓存。',
      currentSourceEffective: '当前生效数据源',
      currentSourceEffectiveDesc: '其他页面切换到 Personal 后，实际使用的是哪种数据源模式。',
      currentSourceYears: '已接入火星年',
      currentSourceYearsDesc: '当前真正可以在其他页面被 Personal 模式调用的火星年数量。',
      currentSourceEmpty: '当前还没有可用的个人数据源火星年。',
      currentSourceHint: '上传通过后，系统会自动尝试把数据接入个人数据源；若条件不足，则继续回退到默认系统数据源。',
      currentSourceUseHint: '当数据概览、预测分析或模型训练切换到 Personal 模式时，会使用这里显示的生效数据源。',
      cardType: '类型',
      cardMarsYear: '火星年',
      cardLs: 'Ls',
      cardGrid: '网格',
      ...
      rejectedLabel: '拒绝原因',
      viewerTitle: '数据内容预览 DATA PREVIEW',
      buildStatus: {
        idle: '尚未构建',
        building: '处理中',
        ready: '已可使用',
      ...
      datasetStateDesc: {
        ...
        rejected: '该文件未通过公共审核，不能并入平台默认数据集；但若已接入个人链路，上传者本人仍可在 Personal 模式中继续使用。',
        rejectedMixed: '该文件未通过公共审核，不能并入平台默认数据集；但你的个人 MCD 仍可继续参与混合数据源，供上传者本人在 Personal 模式中使用。',
        failed: '后台在构建个人数据源时失败，当前仍会回退到默认系统数据源。',
        building: '上传已通过，系统正在构建个人数据源，完成后其他页面即可尝试切换使用。',
        personal: '该数据已经作为个人数据源的一部分，可以在数据概览、分析、预测和训练中被 Personal 模式使用。',
        mixed: '该数据已经进入混合模式，系统会使用默认 OpenMARS 与你的 MCD 共同支撑其他页面。',
        fallback: '当前混合模式下使用的是系统 OpenMARS，这份 OpenMARS 文件本身没有被 Personal 模式采用。',
  ...
  predict: {
    ...
    hud: {
      ...
      latitudinalProfile: '纬度剖面 LATITUDINAL PROFILE',
      samplingZonal: '采样：经向平均 · 范围：[-90, 90]',
      coreOs: '核心系统 [v6.1.25] · 状态：监控中',
      coreOsLabel: '核心系统',
    },
    sidebar: {
      predictionControl: '预测控制 PREDICTION CONTROL',
      parameters: '参数设置 PARAMETERS',
      inputVariables: '输入变量 INPUT VARIABLES',
    },
    display: {
    ...
    shapleyMarginalBtn: '🧬 组合边际分析 (SEED 32)',
    shapleyGeneratingBtn: '正在计算 SHAP 全局归因...',
    barChart: {
      analytics: 'ANALYTICS // 分析系统',
      spectrum: 'PERFORMANCE SPECTRUM // 性能频谱',
      topModel: '最优模型 TOP MODEL',
    ...
    shapley: {
      ...
      cacheMissingDesc: '当前指标的 Shapley 边际贡献需要重新遍历计算流...',
      importanceTitle: '特征重要性 (MEAN |SHAP|)',
      meanShap: '平均 SHAP 值 (对预测强度的影响)',
      swarmTitle: '摘要蜂群图 (SUMMARY PLOT)',
      impactOnPred: '对预测值的影响 (SHAP value)',
      marginalAnalysisTitle: '特征边际贡献 (SHAPLEY VALUE)',
      avgContribution: ({ metric }) => `平均边际贡献 (${metric})`,
      mathPrinciple: '数学原理',
      mathDesc: ({ metric }) => `基于博弈论的 Shapley Value 计算特征对模型 ${metric} 的独立边际贡献。`,
      mathDescNote: {
      ...
      lowValue: '特征低值',
      highValue: '特征高值',
      footerNote: '数据基于测试集全样本计算 • 红/蓝点表示特征原始数值的高/低\nSHAPLEY VALUES 提供了模型预测逻辑的全局透明度解释',
      featureValue: '特征数值',
    },
  },
