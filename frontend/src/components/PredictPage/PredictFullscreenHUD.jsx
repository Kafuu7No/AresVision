      ...
      <div style={{ 
        ...
        {stepLs != null && (
          <div style={{ marginTop: 16, display: 'flex', flexDirection: 'column', gap: 4 }}>
            <div style={{ fontSize: 10, color: C.ice30, fontFamily: "'Orbitron', sans-serif" }}>SOLAR LONGITUDE</div>
            <div style={{ fontSize: 16, color: C.blue, fontWeight: 700 }}>Ls {stepLs.toFixed(3)}°</div>
          </div>
        )}
        
        <div style={{ marginTop: 12, display: 'flex', flexDirection: 'column', gap: 4 }}>
          <div style={{ fontSize: 10, color: C.ice30, fontFamily: "'Orbitron', sans-serif" }}>ATMOSPHERIC OZONE</div>
          <div style={{ fontSize: 16, color: C.mars, fontWeight: 700 }}>
            {fullscreen3D.fieldData.minVal != null ? fmtNum(convertOzone(fullscreen3D.fieldData.minVal, ozoneUnit), precision) : '—'} 
            <span style={{ fontSize: 12, color: C.ice30, margin: '0 6px' }}>to</span>
            {fullscreen3D.fieldData.maxVal != null ? fmtNum(convertOzone(fullscreen3D.fieldData.maxVal, ozoneUnit), precision) : '—'}
            <span style={{ fontSize: 10, color: C.ice30, marginLeft: 4 }}>{ozoneLabel(ozoneUnit)}</span>
          </div>
        </div>
      </div>
    </div>
